...
). Make sure the file *capital_cities.csv* is in your current working directory. Execute the following code to obtain the objects `coordinates_capital_cities` and `country_names`. 
...
3. Subtract `coordinates_capital_cities` from the matrix created in (2). Element-wise subtraction will occur.
...
5. Using the distance obtained above, find the country that has the closest capital to DC.
