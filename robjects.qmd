...
When we use arithmetic operators like `+`, `-`, `*`, etc., or comparison operators like `>`, `>=`, `==`, etc., between atomic vectors, then these operators are applied element-wise on each pair of elements of the atomic vectors with the same index. Consider the examples below.
