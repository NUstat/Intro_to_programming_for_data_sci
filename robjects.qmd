...
The `sapply()` function is used to apply a function on all elements of a list, atomic vector or matrix.

For example, consider a vector below:
...
Write a function that identifies if a word is a [palindrome](https://en.wikipedia.org/wiki/Palindrome) *(A palindrome is a word that reads the same both backwards and forwards, for example, peep, rotator, madam, etc.)*. Apply the function to the vector of words below to count the vector of palindrome words.
...
{
  reversed_word<-''
  for(c in 1:nchar(word))
  {
    reversed_word<-paste0(substr(word,c,c),reversed_word)
  }
  if(reversed_word==word){return(TRUE)}else{return(FALSE)}
}
...
dc_coord<-coordinates_capital_cities[which(country_names=='United States'),]
diff = coordinates_capital_cities-matrix(c(rep(dc_coord,245)),245,2,byrow = TRUE)
distances = apply(diff**2,1,sum)
distances[distances==0]<-99999
closest_country<-country_names[which.min(distances)]
print(closest_country)
coord_close<-coordinates_capital_cities[which.min(distances),]
```
